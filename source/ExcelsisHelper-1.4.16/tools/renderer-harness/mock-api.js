(function installRendererHarness() {
  ...
  function settingsResult(extra = {}) {
    return {
      ...
      defaultErpUserName: "Mock Windows User",
      macroSettings: {
        synced: true,
        settingsPath: "C:\\Mock\\Excelsis Helper\\Macros\\macro-settings.json",
        warnings: [],
      },
      ...extra,
    };
  ...
  const methods = {
    ...
    solidWorksStatus: async () => ({
      ...
      solidWorksConnection: { state: "connected", label: "SW COM: connected", canKill: true },
      activeDocument: {
    ...
    listRecentDocs: async ({ filter = "all", query = "", showAll = false } = {}) => {
      ...
      return {
        ...
        canToggleAll: !normalizedQuery && filtered.length > 50,
        showAll,
        limit: 50,
      };
    },
    copyDocumentPath: async (docPath, options = {}) => {
      record("copyDocumentPath", [docPath, options]);
      const includeDocumentName = options?.includeDocumentName !== false;
      const clipboardText = includeDocumentName
        ? docPath
        : String(docPath || "").replace(/[\\/][^\\/]*$/, "");
      return { ok: true, path: docPath, includeDocumentName, clipboardText, copiedToClipboard: true };
    },
    ...
    retryRecentDocThumbnail: async (docPath) => {
      record("retryRecentDocThumbnail", [docPath]);
      if (new URLSearchParams(location.search).get("viewport") === "fail") {
        return { ok: false, error: "Activate the requested document in SOLIDWORKS, frame its view, then retry." };
      }
      return { ok: true, regenerated: true, viewportOnly: true, path: docPath };
    },
    ...
    listWorklogs: async () => ({
      ...
      counterStatus: {
        isCounting: false,
        code: settings.erp.workLoggingEnabled === false ? "disabled" : "waiting",
        headline: settings.erp.workLoggingEnabled === false ? "Work logging is disabled" : "Nothing is being counted",
        message: settings.erp.workLoggingEnabled === false ? "Work logging and export are disabled in Settings." : "Renderer harness idle state.",
      },
      autoExport: {
        enabled: settings.erp.workLoggingEnabled !== false,
