...
const DEFAULT_OVERTIME_START_MINUTES = 17 * 60;

function normalizeOvertimeStartMinutes(value) {
  const minutes = Number(value);
  if (!Number.isFinite(minutes)) return DEFAULT_OVERTIME_START_MINUTES;
  return Math.max(0, Math.min((24 * 60) - 1, Math.round(minutes)));
}

// Split a sampled interval against the local daily overtime boundary. This is
// deliberately independent from polling cadence and handles midnight/DST by
// constructing each local day's boundary instead of assuming 24-hour days.
function splitElapsedAtLocalCutoff(endAt, elapsedMs, overtimeStartMinutes) {
  const end = Number(endAt);
  const duration = Math.max(0, Number(elapsedMs || 0));
  if (!Number.isFinite(end) || end <= 0 || duration <= 0) {
    return { regularMs: 0, overtimeMs: 0 };
  }

  const cutoffMinutes = normalizeOvertimeStartMinutes(overtimeStartMinutes);
  ...
  while (cursor < end && guard < 370) {
    ...
    overtimeMs += Math.max(0, segmentEnd - Math.max(cursor, cutoff));
    if (segmentEnd <= cursor) break;
    cursor = segmentEnd;
    guard += 1;
  }
  overtimeMs = Math.max(0, Math.min(duration, overtimeMs));
  return {
    regularMs: duration - overtimeMs,
    overtimeMs,
  };
}
...
class UnsavedWorkTracker {
  ...
  pause() {
    // Keep already-earned provisional time, but never bridge a disabled interval
    // or infer a Save As relink from a selection made before logging was paused.
    this.lastSample = { at: 0, key: "", counting: false };
    this.lastUnsaved = { at: 0, key: "" };
  }
  ...
  observe(input = {}) {
    ...
    if (!docPath) {
      ...
      this.addElapsedToEntry(entry, now, elapsedMs, input.overtimeStartMinutes);
    ...
    return finish({
      ...
      promoteRegularMs: Math.max(0, Number(entry.regularMs || 0)),
      promoteOvertimeMs: Math.max(0, Number(entry.overtimeMs || 0)),
      promotionMinMs: this.promotionMinMs,
      docTitle: entry.docTitle,
      docType: entry.docType,
    });
  ...
  addElapsedToEntry(entry, now, elapsedMs, overtimeStartMinutes) {
    const availableMs = Math.max(0, this.maxPendingMs - Math.max(0, Number(entry.totalMs || 0)));
    const acceptedMs = Math.min(availableMs, Math.max(0, Number(elapsedMs || 0)));
    if (acceptedMs <= 0) return;
    const split = splitElapsedAtLocalCutoff(now, acceptedMs, overtimeStartMinutes);
    entry.totalMs = Math.max(0, Number(entry.totalMs || 0)) + acceptedMs;
    entry.regularMs = Math.max(0, Number(entry.regularMs || 0)) + split.regularMs;
    entry.overtimeMs = Math.max(0, Number(entry.overtimeMs || 0)) + split.overtimeMs;
  }
