...
; copies bundled compiled macros into the logged-in user's Documents folder,
; backs up differing existing macros before replacement, and lets the app move
; explicitly retired macro names into its preserved macrobackup tree.
;
...
!macro customInstall
  ...
  ; settings. Existing settings always win. The app reports locked macro files
  ; with a close-SOLIDWORKS / Retry / Later warning under the correct user.
  ; Silent installs never launch the app or show the post-install dialog.
  ${IfNot} ${Silent}
    ${StdUtils.ExecShellAsUser} $0 "$launchLink" "open" "--after-install"
  ${EndIf}
!macroend
