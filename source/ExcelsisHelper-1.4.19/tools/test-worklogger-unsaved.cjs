...
{
  const endAt = new Date(2026, 7, 5, 17, 0, 5, 0).getTime();
  const split = splitElapsedAtLocalCutoff(endAt, 10000, 17 * 60);
  assert.equal(split.regularMs, 5000);
  assert.equal(split.overtimeMs, 5000);
}

{
  const tracker = new UnsavedWorkTracker();
  const startAt = new Date(2026, 7, 5, 16, 59, 20, 0).getTime();
  const { now } = accrue(
    tracker,
    startAt,
    [10000, 10000, 10000, 10000, 10000, 10000],
    { overtimeStartMinutes: 17 * 60 },
  );
  const promoted = tracker.observe(input({
    now,
    docPath: "C:\\CompanyProjects\\PRJ-26-001\\AcrossCutoff.SLDPRT",
    docTitle: "AcrossCutoff.SLDPRT",
    eligibleSavedPath: true,
    overtimeStartMinutes: 17 * 60,
  }));
  assert.equal(promoted.kind, "promote");
  assert.equal(promoted.promoteMs, 60000);
  assert.equal(promoted.promoteRegularMs, 40000);
  assert.equal(promoted.promoteOvertimeMs, 20000);
}
...
const root = path.join(__dirname, "..");
{
  ...
  const { now } = accrue(tracker, 1000, [10000, 10000, 10000, 10000, 10000, 10000]);
  tracker.pause();
  assert.equal(tracker.snapshot().pending[0].totalMs, 60000);
  assert.equal(tracker.snapshot().lastUnsaved.key, "");
  const resumed = tracker.observe(input({ now: now + 3600000 }));
  assert.equal(resumed.elapsedMs, 0);
  const promoted = tracker.observe(input({
    now: now + 3600000,
    docPath: "C:\\Projects\\Part4.SLDPRT",
    eligibleSavedPath: true,
  }));
  assert.equal(promoted.kind, "promote");
  assert.equal(promoted.promoteMs, 60000);
