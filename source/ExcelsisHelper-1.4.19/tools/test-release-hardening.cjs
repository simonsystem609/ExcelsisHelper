...
for (const source of [laserDxf, laserDxfReadOnly]) {
  ...
  assert.match(source, /Function BuildSelectedQtyDict\(selectedCandidates As Collection\) As Object/);
  ...
  assert.match(source, /Set scopedParts = NormalizeVisiblePartScopes\(visibleParts, selectedQty\)/);
  ...
  assert.match(source, /contextInfo = Array\(assemblyPath, cfg, Nothing, Nothing\)/);
  assert.match(source, /Set contextInfo\(2\) = mergedParts/);
  assert.match(source, /BuildVisibleSubassemblyPartCandidates\(swComp\)/);
  assert.match(source, /AddDxfOccurrence dictCandidates, key, candidate/);
  assert.match(source, /MergeDxfOccurrences occurrences, visibleParts/);
  assert.doesNotMatch(source, /\bassemblyCopies\b/);
...
assert.match(pkg.version, /^1\.4\.19$/);
