...
const startupAfterInstall = process.argv.includes("--after-install");
...
function automationMacroRunWatcherGatePath() {
  return path.join(os.tmpdir(), "excelsis-helper-macro-gate.json");
}
...
function thumbPathForDoc(docPath) {
  ...
  const rendererSalt = [".dxf", ".dwg"].includes(ext)
    ? "|cad-render-thumb-v2"
    : ext === ".slddrw"
      ? "|drawing-thumb-v2-negative-preview-fix"
      : "|thumb-v1";
...
const manualViewportThumbPaths = new Set();
let manualViewportThumbsLoaded = null;

function loadManualViewportThumbnails() {
  if (!manualViewportThumbsLoaded) {
    manualViewportThumbsLoaded = fs.readdir(recentDocsThumbDir()).then((names) => {
      for (const name of names) {
        if (/^[a-f0-9]{20}\.png\.manual\.png$/i.test(name)) {
          manualViewportThumbPaths.add(path.join(recentDocsThumbDir(), name.slice(0, -11)));
        }
      }
    }).catch(() => {});
  }
  return manualViewportThumbsLoaded;
}

async function readThumbnailState(thumbPath) {
  await loadManualViewportThumbnails();
  if (manualViewportThumbPaths.has(thumbPath)) {
    const manualPath = `${thumbPath}.manual.png`;
    const state = await readThumbnailFileState(manualPath);
    if (!state.missing && !state.blank) return { ...state, path: manualPath, manual: true };
  }
  return { ...await readThumbnailFileState(thumbPath), path: thumbPath, manual: false };
}

async function readThumbnailFileState(thumbPath) {
  let st;
...
//   D:\Engineering\bar.sldprt                      -> D:\Engineering\bar.sldprt
...
const DOC_SEARCH_SCHEMA_VERSION = 4; // Rebuild once to exclude NAS recycle folders from old indexes.
...
const RECENT_DOC_REPAIR_INTERVAL_MS = 90 * 1000;
const RECENT_DOC_REPAIR_STARTUP_DELAY_MS = 12 * 1000;
const RECENT_DOC_REPAIR_TOP_LIMIT = RECENT_DOC_DEFAULT_LIST_LIMIT;
const RECENT_DOC_MISSING_CONFIRM_CHECKS = 2;
const RECENT_DOC_MISSING_CONFIRM_GAP_MS = 60 * 1000;
const RECENT_DOC_MISSING_CONFIRM_AGE_MS = 2 * 60 * 1000;
const RECENT_DOC_REPAIR_TARGET_SCANS_PER_PASS = 1;
...
function applyLocationSettings(settings) {
  ...
  applyWorkLoggingSettings(settings);
...
async function shouldSkipSolidWorksThumbnailTier() {
  if (await isMacroRecentDocSuppressionActive()) return true;
  const activity = await readActivityWatcherStatus();
  return isSolidWorksForegroundActivity(activity);
}
...
async function runThumbnailExtractionBatch(entries, options = {}) {
  ...
  for (const entry of entries) {
    ...
    const thumbState = await readThumbnailState(outPng);
    if (thumbState.manual) continue;
    ...
    pairs.push({ path: entry.path, outPng });
  ...
  try {
    ...
    // embedded preview fall through to the PowerShell tiers. Explicit viewport
    // capture uses its own one-shot route below.
    let remaining = pairs;
    {
    ...
    const skipSolidWorksTier = await shouldSkipSolidWorksThumbnailTier();
    logActivity("thumb-batch-start", { count: remaining.length, force, skipShellTier, skipSolidWorksTier });
    let stdout = "";
    await new Promise((resolve) => {
      execFile(POWERSHELL_EXE, [
        ...
        "-InputJson", tmpJson,
        "-ThumbSize", "256",
        "-MacroRunMarkerPath", automationMacroRunMarkerPath(),
        "-MaxSolidWorksApiCalls", "1",
        ...(skipShellTier ? ["-SkipShellTier"] : []),
        ...(skipSolidWorksTier ? ["-SkipSolidWorksTier"] : []),
...
async function ensureOpenDocThumbnailsFromStatus(bridgeResult) {
  ...
  for (const doc of docs) {
    ...
    let stale = false;
    if (!missing && !blank && !thumbState.manual) stale = docStat.mtimeMs > thumbState.mtimeMs;
  ...
  if (entries.length) {
    ...
    // Bulk retry only uses saved previews and never changes the user's view.
...
// A manual capture never queues behind disk preview work or changes the user's
// model view. Its separate cache wins over any in-flight/background extraction.
async function scheduleSingleDocThumbnailRetry(docPath) {
  const target = String(docPath || "").trim();
  if (!/\.(sldprt|sldasm|slddrw)$/i.test(target)) {
    return { ok: false, error: "Viewport capture requires a SOLIDWORKS document." };
  }
  if (manualViewportCaptureInProgress || await isMacroRecentDocSuppressionActive()) {
    return { ok: false, error: "A macro or viewport capture is already running. Retry after it finishes." };
  }
  // Recheck after the asynchronous macro check to keep rapid clicks single-flight.
  if (manualViewportCaptureInProgress) return { ok: false, error: "Viewport capture is already running." };
  manualViewportCaptureInProgress = true;
  beginHelperMacroRun();
  const key = target.toLowerCase();
  try {
    await publishMacroRunWatcherGate(true);
    const stagingRoot = path.join(recentDocsThumbDir(), "viewport-staging");
    await fs.mkdir(recentDocsThumbDir(), { recursive: true });
    await fs.mkdir(stagingRoot, { recursive: true });
    const stagingPng = path.join(stagingRoot, "viewport-capture.png");
    const result = await new Promise((resolve) => {
      execFile(POWERSHELL_EXE, hiddenPowerShellArgs(
        "-File", assetPath("scripts", "capture-sw-viewport.ps1"),
        "-DocumentPath", target, "-OutputPng", stagingPng,
        "-MacroRunMarkerPath", automationMacroRunMarkerPath(),
      ), { windowsHide: true, timeout: 20000, maxBuffer: 1024 * 1024 }, (error, stdout) => {
        if (error) return resolve({ ok: false, error: "Viewport capture failed or timed out. Existing thumbnail kept." });
        try { resolve(JSON.parse(String(stdout || ""))); }
        catch { resolve({ ok: false, error: "Viewport capture returned no usable result. Existing thumbnail kept." }); }
      });
    });
    if (!result?.ok) return result;
    blankThumbVerdictCache.delete(stagingPng);
    const captured = await readThumbnailFileState(stagingPng);
    if (captured.missing || captured.blank) return { ok: false, error: "The viewport capture was empty. Existing thumbnail kept." };
    const thumbPath = thumbPathForDoc(target);
    const manualPath = `${thumbPath}.manual.png`;
    if (await pathExists(manualPath)) {
      await fs.copyFile(manualPath, `${manualPath}.${Date.now()}.previous`);
    }
    await fs.rename(stagingPng, manualPath);
    manualViewportThumbPaths.add(thumbPath);
    blankThumbVerdictCache.delete(manualPath);
    thumbRetryState.delete(key);
    clearThumbnailRetryTimer(key);
    sendAutomationRendererEvent("automation:thumbnails-changed", { paths: [target] });
    return { ok: true, regenerated: true, viewportOnly: true, method: "sw-viewport", path: target };
  } catch (error) {
    return { ok: false, error: `Could not replace the viewport thumbnail: ${error.message}` };
  } finally {
    endHelperMacroRun();
    manualViewportCaptureInProgress = false;
    await publishMacroRunWatcherGate(await isMacroRecentDocSuppressionActive());
  }
}
...
async function scanDocSearchTarget(seedPath, knownRoots = null) {
  const roots = Array.isArray(knownRoots)
    ? knownRoots.map((root) => path.normalize(String(root || ""))).filter(Boolean)
    : await docSearchTargetRoots(seedPath);
...
async function publishMacroRunWatcherGate(active) {
  const payload = {
  ...
  try {
    await fs.writeFile(automationMacroRunWatcherGatePath(), JSON.stringify(payload), "utf8");
  } catch {}
}
...
let workLoggingEnabled = true;

function isWorkLoggingEnabled(settings) {
  return settings?.erp?.workLoggingEnabled !== false;
}

function workLoggingDisabledResult() {
  return { ok: false, disabled: true, error: "Work logging and export are disabled in Settings." };
}

function defaultErpWorklogUserName() {
  try { return cleanString(os.userInfo().username) || "User"; } catch { return "User"; }
}

function erpWorklogUserName(settings) {
  return cleanString(settings?.erp?.worklogUserName) || defaultErpWorklogUserName();
}

function applyWorkLoggingSettings(settings) {
  const enabled = isWorkLoggingEnabled(settings);
  if (enabled === workLoggingEnabled) return;
  workLoggingEnabled = enabled;
  resetProjectActivitySample();
  clearRememberedWorkLoggerDocument();
  unsavedWorkTracker.pause();
...
function saveRecentDocs() {
  ...
  // Preserve call order when a heartbeat and the low-rate repair pass both save:
...
function scoreRecentDocRepairCandidate(entry, candidate) {
  ...
  const oldSize = Number(entry?.size || 0);
  const candidateSize = Number(candidate?.size || 0);
  const oldMtime = Number(entry?.mtimeMs || 0);
  const candidateMtime = Number(candidate?.mtimeMs || 0);
  let score = 0;
  ...
  if (
    oldInfo.relativeInsideProject
    && oldInfo.relativeInsideProject.toLowerCase() === candidateInfo.relativeInsideProject.toLowerCase()
  ) score += 120;
  ...
  if (oldSize > 0 && candidateSize === oldSize) score += 80;
  if (oldMtime > 0 && candidateMtime > 0 && Math.abs(candidateMtime - oldMtime) <= 2000) score += 100;
  if (path.basename(candidatePath).startsWith("~$")) score -= 500;
  return score;
}

function recentDocCandidateHasFingerprint(entry, candidate) {
  const size = Number(entry?.size || 0);
  const mtimeMs = Number(entry?.mtimeMs || 0);
  return size > 0
    && mtimeMs > 0
    && Number(candidate?.size || 0) === size
    && Math.abs(Number(candidate?.mtimeMs || 0) - mtimeMs) <= 2000;
}

async function findRecentDocInCandidates(entry, candidates) {
  ...
  if (!name || !ext) return "";
  const oldKey = docSearchPathKey(oldPath);
  const oldRoot = rootOfPath(oldPath).toLowerCase();
  ...
  const eligible = source.filter((candidate) => {
    ...
    const candidateExt = String(candidate?.ext || path.extname(candidatePath)).toLowerCase();
    if (!candidatePath || candidateExt !== ext || docSearchPathKey(candidatePath) === oldKey) return false;
    if (shouldExcludeDocPath(candidatePath)) return false;
    const candidateName = String(candidate?.name || path.basename(candidatePath)).toLowerCase();
    return candidateName === name || recentDocCandidateHasFingerprint(entry, candidate);
  });

  // A Windows file ID is meaningful only within its volume. Requiring the
  // same visual root avoids cross-drive inode collisions.
  if (fileId && fileId !== "0") {
    const byFileId = source
      .filter((candidate) => {
        const candidatePath = String(candidate?.path || "").trim();
        return candidatePath
          && String(candidate?.fileId || "") === fileId
          && String(candidate?.ext || path.extname(candidatePath)).toLowerCase() === ext
          && rootOfPath(candidatePath).toLowerCase() === oldRoot
          && docSearchPathKey(candidatePath) !== oldKey
          && !shouldExcludeDocPath(candidatePath);
      })
      .sort((left, right) => scoreRecentDocRepairCandidate(entry, right) - scoreRecentDocRepairCandidate(entry, left));
    for (const candidate of byFileId) {
  ...
  for (const candidate of ranked) {
    if (candidate.score < 200 || live.length >= 2) break;
    if (await pathExists(candidate.path)) live.push(candidate);
  }
  if (!live.length) return "";
  if (live.length > 1 && live[0].score === live[1].score) return "";
  return live[0].path;
}

async function findRecentDocInIndex(entry, index = null) {
  const source = index || await loadDocSearchIndex();
  return findRecentDocInCandidates(entry, source.entries || []);
}

async function findMovedRecentDoc(entry, context) {
  const index = context.index || await loadDocSearchIndex();
  context.index = index;
  const indexed = await findRecentDocInIndex(entry, index);
  if (indexed) return { path: indexed, searched: true, source: "index" };

  // The first confirmed miss may run one bounded targeted Doc Search crawl.
  // Overlapping project/year roots share the result. An entry cannot collect
  // missing confirmations while its targeted search was deferred by the pass
  // budget; a normal index rebuilt after the miss also counts as a search.
  const searchRequestedAt = Number(entry?.missingSearchRequestedAt || 0);
  const indexUpdatedAt = Number(index?.updatedAt || index?.generatedAt || 0);
  if (Number(entry?.missingChecks || 0) > 0 || (searchRequestedAt > 0 && indexUpdatedAt >= searchRequestedAt)) {
    return { path: "", searched: true, source: "refreshed-index" };
  }
  const roots = await docSearchTargetRoots(entry?.path || "");
  if (!roots.length) return { path: "", searched: false, source: "no-search-root" };
  const rootKeys = roots.map((root) => docSearchPathKey(root));
  let targetResult = null;
  for (const key of rootKeys) {
    if (context.targetResults.has(key)) {
      targetResult = context.targetResults.get(key);
      break;
    }
  }
  if (!targetResult) {
    if (context.targetScans >= RECENT_DOC_REPAIR_TARGET_SCANS_PER_PASS) {
      return { path: "", searched: false, source: "target-scan-deferred" };
    }
    context.targetScans++;
    targetResult = await scanDocSearchTarget(entry.path, roots);
    for (const root of targetResult.roots || roots) {
      context.targetResults.set(docSearchPathKey(root), targetResult);
    }
  }
  return {
    path: await findRecentDocInCandidates(entry, targetResult.entries || []),
    searched: true,
    source: "target-scan",
  };
}

async function observeRecentDocPath(filePath) {
  try {
    const st = await fs.stat(filePath);
    if (!st.isFile()) return { kind: "missing", errorCode: "NOT_FILE" };
    return {
      kind: "present",
      fileId: st.ino ? String(st.ino) : "",
      size: Number(st.size || 0),
    ...
    return {
      kind: ["ENOENT", "ENOTDIR"].includes(errorCode) ? "missing" : "unavailable",
      errorCode,
    };
  }
}

function recentDocStorageRoot(filePath) {
  const parsedRoot = path.parse(String(filePath || "")).root;
  if (parsedRoot) return parsedRoot;
  const visualRoot = rootOfPath(filePath);
  return /^[A-Za-z]:$/.test(visualRoot) ? `${visualRoot}\\` : visualRoot;
}

async function recentDocStorageAvailable(filePath, rootStates) {
  const root = recentDocStorageRoot(filePath);
  if (!root) return false;
  const key = docSearchPathKey(root);
  if (!rootStates.has(key)) {
    rootStates.set(key, (async () => {
      try {
        const st = await fs.stat(root);
  ...
  try {
    ...
    if (!existing.length) {
      return {
        ...
        skipped: [],
        count: 0,
        limit: RECENT_DOC_REPAIR_TOP_LIMIT,
      };
    }
    const selected = existing.slice(0, RECENT_DOC_REPAIR_TOP_LIMIT);
    ...
    const removed = [];
    const skipped = [];
    const maintenanceUpdates = new Map();
    const removedEntryVersions = new Map();
    const rootStates = new Map();
    const repairContext = { index: null, targetResults: new Map(), targetScans: 0 };
    let docSearchRefreshRequested = false;
    ...
    for (let selectedIndex = 0; selectedIndex < selected.length; selectedIndex++) {
      ...
      if (selectedIndex > 0 && selectedIndex % 10 === 0) await sleep(10);
      ...
      if (!(await recentDocStorageAvailable(entryPath, rootStates))) {
        skipped.push({ path: entryPath, reason: "storage-unavailable", errorCode: "ROOT_UNAVAILABLE" });
        continue;
      }
      const observation = await observeRecentDocPath(entryPath);
      if (observation.kind === "present") {
        const evaluated = evaluateRecentDocObservation(entry, observation, Date.now(), {
          requiredChecks: RECENT_DOC_MISSING_CONFIRM_CHECKS,
          minCheckGapMs: RECENT_DOC_MISSING_CONFIRM_GAP_MS,
          minMissingAgeMs: RECENT_DOC_MISSING_CONFIRM_AGE_MS,
        });
        if (evaluated.changed) {
          next[currentIndex] = evaluated.entry;
          maintenanceUpdates.set(entryKey, {
      ...
      if (observation.kind === "unavailable") {
        skipped.push({ path: entryPath, reason: "storage-unavailable", errorCode: observation.errorCode || "" });
        continue;
      }
      checkedMissing++;
      const moveResult = await findMovedRecentDoc(entry, repairContext);
      let movedPath = moveResult.path;
      let movedObservation = null;
      if (movedPath) {
        movedObservation = await observeRecentDocPath(movedPath);
        if (movedObservation.kind !== "present") movedPath = "";
      }
      ...
      if (movedPath && movedKey !== entryKey) {
        ...
        const repairedEntry = {
          ...entry,
          path: movedPath,
          type: classifyDocType(movedPath),
          title: path.basename(movedPath),
          relocatedFrom: entryPath,
          relocatedAt: Date.now(),
        };
        const evaluated = evaluateRecentDocObservation(repairedEntry, movedObservation, Date.now());
        next[currentIndex] = evaluated.entry;
        knownPathKeys.delete(entryKey);
        ...
        if (!moveResult.searched) {
          let deferredEntry = entry;
          if (!entry?.missingSearchRequestedAt) {
            if (!docSearchRefreshRequested) {
              docSearchRefreshRequested = true;
              scheduleDocSearchBackgroundScan(DOC_SEARCH_WATCH_DEBOUNCE_MS);
            }
            deferredEntry = { ...entry, missingSearchRequestedAt: Date.now() };
            next[currentIndex] = deferredEntry;
            maintenanceUpdates.set(entryKey, {
              entry: deferredEntry,
              expectedLastSeen: Number(entry?.lastSeen || 0),
            });
          }
          skipped.push({ path: entryPath, reason: moveResult.source || "target-scan-deferred" });
          continue;
        }
        const evaluated = evaluateRecentDocObservation(entry, { kind: "missing" }, Date.now(), {
          requiredChecks: RECENT_DOC_MISSING_CONFIRM_CHECKS,
          minCheckGapMs: RECENT_DOC_MISSING_CONFIRM_GAP_MS,
          minMissingAgeMs: RECENT_DOC_MISSING_CONFIRM_AGE_MS,
        });
        if (evaluated.action === "remove") {
          next.splice(currentIndex, 1);
          knownPathKeys.delete(entryKey);
          removedEntryVersions.set(entryKey, Number(entry?.lastSeen || 0));
          removed.push({ path: entryPath, reason: "confirmed-missing" });
        } else if (evaluated.changed) {
          let pendingEntry = evaluated.entry;
          if (!entry?.missingSearchRequestedAt) {
            if (!docSearchRefreshRequested) {
              docSearchRefreshRequested = true;
              scheduleDocSearchBackgroundScan(DOC_SEARCH_WATCH_DEBOUNCE_MS);
            }
            pendingEntry = { ...pendingEntry, missingSearchRequestedAt: Date.now() };
          }
          next[currentIndex] = pendingEntry;
          maintenanceUpdates.set(entryKey, {
            entry: pendingEntry,
            expectedLastSeen: Number(entry?.lastSeen || 0),
          });
          skipped.push({
            ...
            missingChecks: Number(pendingEntry?.missingChecks || 0),
          });
        }
      }
    }

    if (maintenanceUpdates.size || removed.length || next.length !== existing.length) {
      if (recentDocsCache === existing) {
        ...
        for (const [fromKey, update] of maintenanceUpdates) {
    ...
    return {
      ...
      limit: RECENT_DOC_REPAIR_TOP_LIMIT,
      targetScans: repairContext.targetScans,
    };
  } finally {
...
function ensureProjectActivityCacheForToday(reason = "date-check") {
  if (!workLoggingEnabled) return { reset: false, changed: false };
...
async function addProjectActivityDuration(docPath, elapsedMs, maxDurationMs, options = {}) {
  if (!workLoggingEnabled) return null;
...
async function adjustProjectActivityMinutes(projectKey, deltaMinutes) {
  if (!workLoggingEnabled) return workLoggingDisabledResult();
  const key = projectActivityKey(projectKey);
...
async function writeWorklogExportEntries(projects, activeDate, rules, workTypesCatalog) {
  ...
  if (!isWorkLoggingEnabled(settings) || !workLoggingEnabled) return workLoggingDisabledResult();
  const inbox = erpWorklogInbox(settings);
  const worklogUserName = erpWorklogUserName(settings);
...
async function exportProjectActivityToErp(rawRules = {}) {
  if (!isWorkLoggingEnabled(await readAutomationSettings()) || !workLoggingEnabled) return workLoggingDisabledResult();
...
function getAutoExportStatusForUi() {
  ...
  return {
    ...
    nextRunAt: workLoggingEnabled ? nextAutoExportRunAt() : null,
  };
}
...
async function attemptMidnightAutoExport(dateKey) {
  if (!workLoggingEnabled || !isWorkLoggingEnabled(await readAutomationSettings())) return;
  ...
  if (!workLoggingEnabled || result?.disabled) return;
...
async function noteRecentDoc(filePath, type, title, options = {}) {
  ...
  let size = Number(existing?.size || 0);
  let mtimeMs = Number(existing?.mtimeMs || 0);
...
function buildWorkLoggerCounterStatus(bridgeResult, decision, details = {}) {
  ...
  if (!bridgeResult?.ok || (!bridgeResult?.connected && !bridgeResult?.trackingFromCache)) {
...
function currentWorkLoggerCounterStatus() {
  if (!workLoggingEnabled) {
    return {
      updatedAt: Date.now(), isCounting: false, code: "disabled",
      headline: "Work logging is disabled",
      message: "Work logging and export are disabled in Settings.",
      docPath: "", docTitle: "", projectName: "",
    };
  }
...
async function trackProjectActivityFromStatus(bridgeResult, options = {}) {
  const now = Date.now();
  const settings = options.settings || await readAutomationSettings();
  if (!workLoggingEnabled || !isWorkLoggingEnabled(settings)) {
    resetProjectActivitySample();
    return { counted: false, isCounting: false, elapsedMs: 0, disabled: true, counterStatus: currentWorkLoggerCounterStatus() };
  }
  const decision = options.decision || shouldCountSolidWorksActivity(bridgeResult, settings);
  const doc = bridgeResult?.activeDocument;
  const docPath = String(doc?.path || "").trim();
  ...
  const unsavedObservation = unsavedWorkTracker.observe({
    ...
    connected: Boolean((bridgeResult?.connected || bridgeResult?.trackingFromCache) && bridgeResult?.ok !== false),
...
function normalizeMacroHotkey(value, fallback = "") {
  ...
  const parts = raw
    ...
    .map((part) => cleanString(part))
    .filter(Boolean);
  if (parts.length < 2 || parts.length > 5) return fallback;
  ...
  const trigger = /^[A-Z0-9]$/.test(triggerRaw)
    ? triggerRaw
...
function mergeAutomationSettings(raw = {}) {
  ...
  return {
    ...
    erp: {
      workLoggingEnabled: erp.workLoggingEnabled !== false,
...
function validateAutomationSettingsPaths(settings) {
  ...
  for (const [label, value] of [
    ["DXF filename prefix", settings.macros?.dxfOutputPrefix],
    ["Default DXF material", settings.macros?.defaultMaterial],
  ]) {
    if (/[<>:"/\\|?*\x00-\x1f]/.test(String(value || ""))) {
      throw new Error(`${label} cannot contain Windows filename separators or reserved characters.`);
    }
  }
  const required = [
...
async function readAutomationSettings(options = {}) {
  ...
  automationSettingsCacheCheckedAt = now;
  applyWorkLoggingSettings(merged);
...
async function retireObsoleteBundledMacroFiles(root, safeVersion, stamp) {
  ...
  for (const fileName of RETIRED_BUNDLED_MACRO_FILES) {
    ...
    try {
      ...
      complete = false;
      warnings.push(`Could not retire ${fileName}: ${error.message}. Check macro-folder write access and close any open macro/VBA editor.`);
      logActivity("macro-retire-failed", { file: fileName, error: error.message });
    }
  }

  return { complete, files: retired, warnings };
}

let bundledMacroDeploymentInFlight = null;
let bundledMacroDeploymentWarnings = [];

async function ensureBundledMacros() {
  if (bundledMacroDeploymentInFlight) return bundledMacroDeploymentInFlight;
  bundledMacroDeploymentInFlight = deployBundledMacros();
  try {
    return await bundledMacroDeploymentInFlight;
  } finally {
    bundledMacroDeploymentInFlight = null;
  }
}

async function deployBundledMacros() {
  ...
  bundledMacroDeploymentWarnings.push(...retirement.warnings);
  try {
    const marker = await readJsonFileNoBom(markerPath);
    if (marker?.appVersion === appVersion && retirement.complete && marker.files?.length) {
      let filesPresent = true;
      for (const fileName of marker.files) {
        const filePath = path.resolve(root, fileName);
        if (!isInsideFolder(filePath, root) || !await pathExists(filePath)) {
          filesPresent = false;
          break;
        }
      }
      if (filesPresent) return root;
    }
  } catch {}
  ...
  if (sourceFiles.length === 0) {
    bundledMacroDeploymentWarnings.push("Bundled compiled macros could not be found. Repair the Helper installation.");
    return root;
  }
  ...
  for (const sourcePath of sourceFiles) {
    ...
    try {
      ...
      complete = false;
      bundledMacroDeploymentWarnings.push(
        `Could not update ${relative}: ${error.message}. Close the macro/VBA editor and retry; if it stays locked, restart SOLIDWORKS and reopen Helper. Check macro-folder write access too.`,
      );
  ...
  if (complete) {
    await fs.writeFile(markerPath, `${JSON.stringify({
      ...
      bundledMacroDeploymentWarnings.push(`Could not record the bundled macro update: ${error.message}`);
    });
  }
  return root;
}
...
let macroSettingsSyncQueue = Promise.resolve();
let lastMacroSettingsSync = null;

function macroSettingsSyncStatus() {
  if (!lastMacroSettingsSync) return null;
  return {
    ...lastMacroSettingsSync,
    warnings: [...lastMacroSettingsSync.settingsWarnings, ...bundledMacroDeploymentWarnings],
  };
}

function applyMacroSettings(settings) {
  const snapshot = mergeAutomationSettings(settings);
  const pending = macroSettingsSyncQueue.then(() => writeMacroRuntimeSettings(snapshot));
  macroSettingsSyncQueue = pending.catch(() => {});
  return pending;
}

async function writeMacroRuntimeSettings(settings) {
  ...
  const settingsPath = path.join(automationMacroRoot(), "macro-settings.json");
  const settingsWarnings = [];
  let synced = false;
  try {
    await ensureBundledMacros();
    ...
    // truncated JSON file. Failed staging files are retained for inspection.
    await fs.writeFile(temporaryPath, content, { encoding: "utf8", flag: "wx" });
    await fs.rename(temporaryPath, settingsPath);
    if (await fs.readFile(settingsPath, "utf8") !== content) {
      throw new Error("Macro settings read-back did not match the saved values.");
    }
    synced = true;
  } catch (error) {
    settingsWarnings.push(`Could not synchronize ${settingsPath}: ${error.message}. Macro defaults have not been confirmed. Check folder write access, then save Settings again.`);
  }

  lastMacroSettingsSync = { ...macroSettings, settingsPath, synced, updated: [], settingsWarnings };
  const result = macroSettingsSyncStatus();
  if (result.warnings.length) logActivity("macro-settings-warning", { warnings: result.warnings });
  return result;
}

let installerMacroUpdateInFlight = null;

async function finishInstallerMacroUpdate(refresh = false) {
  if (installerMacroUpdateInFlight) return installerMacroUpdateInFlight;
  // Setup must use this de-elevated process for the actual user's Documents.
  // Retries are user-driven; neither SOLIDWORKS nor its editor is terminated.
  installerMacroUpdateInFlight = (async () => {
    let status = refresh ? null : macroSettingsSyncStatus();
    for (;;) {
      if (!status) {
        try {
          status = await applyMacroSettings(await readAutomationSettings());
        } catch (error) {
          status = { warnings: [`Could not update macro files or settings: ${error.message}`] };
        }
      }
      const warnings = status.warnings || [];
      if (!warnings.length) return { complete: true, warnings: [] };
      const { response } = await dialog.showMessageBox({
        type: "warning",
        title: "Excelsis Helper setup - macro update",
        message: "Helper was installed, but the macro update did not finish.",
        detail: "Save your work and close SOLIDWORKS, including the VBA editor, then choose Retry. "
          + "If a folder-permission error remains, check write access to the macro folder.\n\n"
          + "Later leaves the affected files unchanged. Existing macros may still be the old version; "
          + "restart Helper after closing SOLIDWORKS to retry. Your saved settings and work logs are kept.\n\n"
          + warnings.join("\n"),
        buttons: ["Retry", "Later"],
        defaultId: 0,
        cancelId: 1,
      ...
      if (response !== 0) return { complete: false, warnings: [...warnings] };
      status = null;
    }
  })();
  try {
    return await installerMacroUpdateInFlight;
  } finally {
    installerMacroUpdateInFlight = null;
  }
}
...
async function listSolidWorksMacros() {
  ...
  return { root, macros, warnings: [...bundledMacroDeploymentWarnings] };
}
...
async function waitForSolidWorksActivePath(targetPath, timeoutMs = 20000) {
  ...
  while (Date.now() - started < timeoutMs) {
    await sleep(500);
    lastStatus = await readSolidWorksWatcherStatus();
  ...
  return {
    ok: false,
    unavailable: connectedSamples === 0,
    elapsedMs: Date.now() - started,
    status: lastStatus,
  };
}
...
async function waitForSolidWorksNoActiveDocument(timeoutMs = 60000) {
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    if (!lastStatus?.connected) {
      ...
      if (unavailableSamples >= 2) {
        return {
          ...
          elapsedMs: Date.now() - started,
          status: lastSolidWorksStatus,
          error: "The SOLIDWORKS COM watcher is not available.",
        };
      }
      await sleep(500);
      continue;
    ...
    const openDocuments = Array.isArray(lastStatus?.openDocuments) ? lastStatus.openDocuments : [];
    if (lastStatus?.ok && !active.hasActiveDocument && openDocuments.length === 0) {
...
function createSolidWorksConnectionState(state, details = {}) {
  const processes = Array.isArray(details.processes) ? details.processes : [];
  const processCount = Number.isFinite(Number(details.processCount))
    ? Math.max(0, Number(details.processCount))
    : processes.length;
  const labels = {
    connected: "SW COM: connected",
    "com-unresponsive": "SW COM: unresponsive",
    disconnected: "SW: not running",
    checking: "SW COM: checking",
  };
  const messages = {
    connected: "The persistent SOLIDWORKS document watcher is connected.",
    "com-unresponsive": "SOLIDWORKS is running, but its COM document watcher is not answering. Last-known work logging can continue during the normal grace period.",
    disconnected: "No running SOLIDWORKS process is available.",
    checking: "Excelsis is waiting for the persistent SOLIDWORKS watcher.",
  };
  return {
    state,
    ok: state === "connected",
    canKill: state === "connected" || processCount > 0,
    label: labels[state] || labels.checking,
    message: details.message || messages[state] || messages.checking,
    reasons: [],
    processCount,
    processes,
    checkedAt: Date.now(),
    signature: `${state}:${processes.map((processInfo) => processInfo.id || "").sort().join(",")}`,
  };
}

function attachSolidWorksConnectionState(status, state, processSnapshot = null, details = {}) {
  const result = status && typeof status === "object"
    ? status
    : { ok: true, connected: false, activeDocument: { hasActiveDocument: false }, openDocuments: [] };
  const processes = normalizeSolidWorksProcessSnapshot(processSnapshot);
  const processCount = state === "connected" && processes.length === 0 ? 1 : processes.length;
  const connection = createSolidWorksConnectionState(state, { ...details, processes, processCount });
  result.connectionState = state;
  result.comResponsive = state === "connected";
  result.solidWorksConnection = connection;
  // Kept as a compatibility alias for existing renderer/harness consumers.
  result.solidWorksHealth = connection;
  if (processSnapshot) result.solidWorksProcessSnapshot = processSnapshot;
  return result;
}

// ---- SOLIDWORKS watch heartbeat + persistent watcher -----------------------
// The foreground helper is Win32-only; the document watcher is the sole
// steady-state SOLIDWORKS COM reader. Both publish one compact status file every
// two seconds. The spawn-per-call bridge is reserved for explicit user actions.
...
let solidWorksHeartbeatInFlight = false;
let lastOpenDocThumbCheckAt = 0;
let lastConnectedSolidWorksStatus = null;
let lastConnectedSolidWorksStatusAt = 0;
...
const SW_WATCHER_INTERVAL_MS = 2000;   // both watcher payloads use one cadence
const SW_WATCHER_FRESH_MS = 8000;      // file older than this => watcher is down
const SW_HEARTBEAT_ACTIVE_MS = 2000;   // watcher-based tick (no spawn)
const SW_PROCESS_REFRESH_MS = 10000;   // Win32 process snapshot only when COM is unavailable
const SW_HEARTBEAT_IDLE_MS = 4000;     // slower retry while SOLIDWORKS is closed
...
function createManagedHelper({
  ...
  function start(config) {
    ...
    if (!plan) return;
    try {
      const onStdoutLine = typeof plan.onStdoutLine === "function" ? plan.onStdoutLine : null;
      const proc = spawn(plan.command, plan.args, {
        windowsHide: true,
        stdio: onStdoutLine ? ["ignore", "pipe", "ignore"] : "ignore",
      });
      helper.proc = proc;
      ...
      if (onStdoutLine && proc.stdout) {
        ...
        proc.stdout.on("end", () => {
          ...
          try { onStdoutLine(stdoutBuffer); } catch {}
          stdoutBuffer = "";
        });
      }
      proc.on("error", () => {
...
async function runMacroFromHotkey(binding) {
  ...
  try {
    ...
    const [macroPath, realMacroRoot] = await Promise.all([
...
const solidWorksWatcherHelper = createManagedHelper({
  ...
  buildSpawn: () => ({
    ...
    args: ["//NoLogo", "//B", assetPath("scripts", "solidworks-watcher.vbs"),
      solidWorksWatcherStatusPath(), String(SW_WATCHER_INTERVAL_MS), automationMacroRunWatcherGatePath()],
  }),
...
const hotkeyHelper = createManagedHelper({
  ...
  // EcoQoS is fine here: all hotkeys use RegisterHotKey/WM_HOTKEY (queued
  ...
  buildSpawn: (hotkeys) => {
    ...
    return {
      ...
      onStdoutLine: handleHotkeyHelperLine,
    };
  },
});
...
// Assemble a status object from the spawn-free watcher file and the Win32-only
// foreground helper. No second COM status bridge runs in the background.
function assembleStatusFromWatcher(watcher, activity) {
  ...
  // powerMonitor idle only; the document watcher remains the sole COM reader.
  ...
  const status = {
    ...
    comResponsive: true,
    macroRunning: watcher.macroRunning === true,
  ...
  return attachSolidWorksConnectionState(status, "connected");
}

// Cache a non-connected status without spending the COM bridge. A running SW
// process retains only the normal bounded last-known-document logging grace.
...
async function refreshDisconnectedSolidWorksProcessSnapshot(now = Date.now()) {
  if (
    ...
    || now - lastDisconnectedSolidWorksProcessPollAt >= SW_PROCESS_REFRESH_MS
  ) {
...
function cacheDisconnectedSolidWorksStatus(snapshot, watcher = null, activity = null) {
  const processes = normalizeSolidWorksProcessSnapshot(snapshot);
  const processRunning = processes.length > 0;
  const state = processRunning ? "com-unresponsive" : (snapshot?.ok ? "disconnected" : "checking");
  const error = processRunning
    ? solidWorksWatcherConnectionError(watcher)
    : (state === "checking" ? String(snapshot?.error || "") : "");
  let windowsActivity = activity;
  if (!windowsActivity) {
    let idleMs = null;
    try { idleMs = Math.round(powerMonitor.getSystemIdleTime() * 1000); } catch {}
    windowsActivity = { ok: true, idleMs, source: "powerMonitor" };
  }
  const lastKnownActiveDocument = lastConnectedSolidWorksStatus?.activeDocument || null;
  const status = {
    ok: true,
    connected: false,
    error,
    solidWorksBusy: processRunning,
    trackingFromCache: Boolean(processRunning && lastKnownActiveDocument?.hasActiveDocument),
    lastKnownAt: lastConnectedSolidWorksStatusAt || 0,
    lastKnownActiveDocument,
    macroRunning: watcher?.macroRunning === true,
    watcherSessionId: String(lastConnectedSolidWorksStatus?.watcherSessionId || ""),
    activeDocument: { hasActiveDocument: false, title: "", path: "", type: "", source: "none" },
    openDocuments: [],
    windowsActivity,
    idleHeartbeat: true,
    fromWatcher: Boolean(watcher),
  };
  attachSolidWorksConnectionState(status, state, snapshot, { message: error || undefined });
  lastSolidWorksStatus = status;
  lastSolidWorksStatusAt = Date.now();
  if (!processRunning && snapshot?.ok) {
...
async function applySolidWorksTracking(status) {
  const settings = await readAutomationSettings();
...
async function solidWorksHeartbeatTick() {
  if (solidWorksHeartbeatInFlight) return;
  solidWorksHeartbeatInFlight = true;
  let nextDelay = SW_HEARTBEAT_ACTIVE_MS;
  try {
    const now = Date.now();
    const macroRunning = await isMacroRecentDocSuppressionActive();
    await publishMacroRunWatcherGate(macroRunning);
    ...
    if (watcher && watcher.connected) {
      ...
      lastConnectedSolidWorksStatus = status;
      lastConnectedSolidWorksStatusAt = now;
      await applySolidWorksTracking(status);
      // Keep the previews of docs currently open in SOLIDWORKS fresh. This is
      // the ONLY place open-doc thumbnails auto-refresh in steady state.
      ...
      if (
        now - lastOpenDocThumbCheckAt >= OPEN_DOC_THUMB_CHECK_INTERVAL_MS
        && !(await isMacroRecentDocSuppressionActive())
      ) {
      ...
      // No usable document payload. Never hammer the same COM apartment with a
      // fallback bridge: keep the last known document on the normal work-log
      // grace, refresh only the Win32 process list, and restart a missing helper.
      const snapshot = await refreshDisconnectedSolidWorksProcessSnapshot(now);
      const status = cacheDisconnectedSolidWorksStatus(snapshot, watcher, activity);
      await applySolidWorksTracking(status);
      if (!watcher) maybeRestartSolidWorksWatcher();
      const swRunning = normalizeSolidWorksProcessSnapshot(snapshot).length > 0;
      nextDelay = swRunning ? SW_HEARTBEAT_ACTIVE_MS : SW_HEARTBEAT_IDLE_MS;
    }
  } catch {} finally {
    solidWorksHeartbeatInFlight = false;
...
function startSolidWorksHeartbeat() {
  ...
  // heartbeat tick runs immediately and reports checking until the watcher's
  // status file appears.
...
async function openDocumentLikeRecentDoc(targetPath, options = {}) {
  ...
  if (opened.ok) {
    ...
    opened.verificationUnavailable = Boolean(opened.verify?.unavailable);
    // Recent Docs and Doc Search use Windows file association to open the
    // document. A temporarily unresponsive COM watcher must not turn that
    // successful launch into a failure or trigger a duplicate second open.
    if (!noFallback) return opened;
  }
  ...
  if (fallback.ok) {
    ...
    opened.verificationUnavailable = Boolean(opened.verify?.unavailable);
    opened.ok = true;
    if (!opened.verified) opened.verificationPending = true;
  }
  return opened;
}
...
trustedIpcHandle("automation:get-settings", async () => {
  ...
  return {
    ...
    defaultErpUserName: defaultErpWorklogUserName(),
    macroSettings: macroSettingsSyncStatus(),
  };
});
...
trustedIpcHandle("automation:solidworks-status", async () => {
  ...
  // window is open. The renderer just reads the cached status, so its 2s poll
  // is free and tracking keeps running while the window is hidden in the tray.
  // The first call before the heartbeat has produced a sample returns a small
  // checking payload instead of spawning a second COM reader.
  if (lastSolidWorksStatus) return lastSolidWorksStatus;
  return attachSolidWorksConnectionState({
    ok: true,
    connected: false,
    activeDocument: { hasActiveDocument: false },
    openDocuments: [],
  }, "checking");
});
...
trustedIpcHandle("automation:copy-current-doc-location", async () => {
  ...
  // is instant and reliable. A temporarily unresponsive COM watcher can still
  // supply its last-known document; this button never starts a competing bridge.
  const status = lastSolidWorksStatus;
  const doc = status?.activeDocument?.hasActiveDocument
    ? status.activeDocument
    : status?.lastKnownActiveDocument;
  try { await noteRecentDocFromStatus(status, { force: true }); } catch {}
  if (!status?.ok || (!status.connected && !status.trackingFromCache)) {
...
trustedIpcHandle("automation:list-recent-docs", async (_event, request = {}) => {
  ...
  const items = await mapWithConcurrency(selected, 6, async (entry) => {
    ...
    if (stillThere) {
      ...
      if (thumbState.blank) {
        ...
        thumbUrl = pathToFileURL(thumbState.path).href + "?v=" + Math.floor(thumbState.mtimeMs);
      }
    }
    return {
      ...entry,
  ...
  return {
    ...
    canToggleAll: !query && filteredCount > RECENT_DOC_DEFAULT_LIST_LIMIT,
    showAll,
...
trustedIpcHandle("automation:retry-recent-doc-thumbnail", async (_event, docPath) => {
  const authorization = await authorizeKnownDocumentPath(docPath);
  if (!authorization.ok) return authorization;
  const target = authorization.requestedPath;
  return scheduleSingleDocThumbnailRetry(target);
});
...
trustedIpcHandle("automation:copy-document-path", async (_event, docPath, options = {}) => {
  const authorization = await authorizeKnownDocumentPath(docPath);
  if (!authorization.ok) return authorization;
  const documentPath = displayPathOf(authorization.requestedPath);
  const includeDocumentName = options?.includeDocumentName !== false;
  const clipboardText = includeDocumentName ? documentPath : path.dirname(documentPath);
...
trustedIpcHandle("automation:kill-solidworks", async () => {
  const processSnapshotBefore = await getSolidWorksProcessSnapshot();
  if (!processSnapshotBefore?.ok) {
    return {
      ok: false,
      error: processSnapshotBefore?.error || "Could not enumerate SLDWORKS.exe processes.",
      processSnapshotBefore,
    };
  }
  const processesBefore = normalizeSolidWorksProcessSnapshot(processSnapshotBefore);
  if (processesBefore.length === 0) {
    return {
      ...
      survivors: [],
      processSnapshotBefore,
      processSnapshotAfter: processSnapshotBefore,
    };
  }
  ...
  const processSnapshotAfter = await getSolidWorksProcessSnapshot();
  const survivors = normalizeSolidWorksProcessSnapshot(processSnapshotAfter);
  const ok = Boolean(result?.ok && processSnapshotAfter?.ok && survivors.length === 0);
  return {
    ...result,
    ok,
    error: ok ? "" : (result?.error || processSnapshotAfter?.error || `${survivors.length} SLDWORKS.exe process(es) survived the force-stop.`),
    survivors: survivors.length ? survivors : (result?.survivors || []),
    processSnapshotBefore,
...
const SOLIDCAM_STATUS_CACHE_MS = 30000;
let swAddinStatusProbeInFlight = null;
let swAddinStatusCache = { signature: "", at: 0, result: null };

function deferredSwAddinStatus(target, reason, connectionState) {
  const state = connectionState === "disconnected" ? "not-running" : "loading";
  const message = connectionState === "disconnected"
    ? "SOLIDWORKS is not running, so the SolidCAM probe is idle."
    : `SolidCAM status probe deferred: ${reason}`;
  return {
    ok: true,
    configured: true,
    ...
    clsid: target.clsid,
    title: target.title,
    solidCamHealth: createSolidCamHealth(state, message, { target }),
  };
}
...
async function runSwAddinStatus(options = {}) {
  ...
  const signature = solidCamTargetSignature(target);
  const now = Date.now();
  if (!force && swAddinStatusCache.signature === signature && now - swAddinStatusCache.at < SOLIDCAM_STATUS_CACHE_MS) {
    return { ...swAddinStatusCache.result, cached: true };
  }

  const macroActive = await isMacroRecentDocSuppressionActive();
  const connectionState = String(lastSolidWorksStatus?.connectionState || "checking");
  const deferredReason = macroActive
    ? "a SOLIDWORKS macro is running"
    : connectionState !== "connected"
      ? `SOLIDWORKS connection state is ${connectionState}`
      : "";
  if (deferredReason) {
    if (swAddinStatusCache.signature === signature && swAddinStatusCache.result) {
      return { ...swAddinStatusCache.result, cached: true, deferred: true, deferredReason };
    }
    return deferredSwAddinStatus(target, deferredReason, connectionState);
  }

  if (swAddinStatusProbeInFlight) {
    const activeProbe = swAddinStatusProbeInFlight;
    const activeResult = await activeProbe.promise;
    if (activeProbe.signature === signature) return activeResult;
  }
  ...
  swAddinStatusProbeInFlight = { signature, promise };
  try {
    const result = await promise;
    swAddinStatusCache = { signature, at: Date.now(), result };
    return result;
  } finally {
    if (swAddinStatusProbeInFlight?.promise === promise) swAddinStatusProbeInFlight = null;
  }
}
...
async function waitForSwAddinLoaded(options = {}, timeoutMs = 90000) {
  ...
  let last = null;
  while (Date.now() - started < timeoutMs) {
    last = await runSwAddinStatus({ ...options, force: true });
    if (last?.loaded === true) {
      return { ok: true, elapsedMs: Date.now() - started, status: last };
    }
    // Loading an add-in is an explicit workflow, but GetAddInObject still
    // enters SOLIDWORKS' single-threaded COM apartment. Five seconds keeps the
    // UI responsive without repeatedly probing a busy/loading application.
    await sleep(5000);
  }
  return {
    ok: false,
...
async function startSolidCamWithNeutralLoaderDocument({ dllPath, clsid }) {
  ...
  result.finalAddinStatus = await runSwAddinStatus({ dllPath, clsid, force: true });
...
async function loadSolidCamAddin(options = {}) {
  ...
  const alreadyLoaded = await runSwAddinStatus({ dllPath: target, clsid, title, force: true });
  ...
  const watcher = await readSolidWorksWatcherStatus();
  const status = watcher?.connected
    ? assembleStatusFromWatcher(watcher, await readActivityWatcherStatus())
    : lastSolidWorksStatus;
  if (!status?.ok || !status.connected) {
    return {
      ok: false,
      error: status?.connectionState === "com-unresponsive"
        ? "SOLIDWORKS is running, but its COM watcher is unresponsive. Wait for it to reconnect before loading SolidCAM."
        : "Open SOLIDWORKS first, then load SolidCAM. Excelsis will not auto-start SOLIDWORKS through COM.",
      status,
    };
  }
...
trustedIpcHandle("automation:run-macro", async (event, options = {}) => {
  ...
  const defaultModule = COMPILED_MACRO_MODULE_NAMES[path.basename(macroPath).toLowerCase()] || "";
...
trustedIpcHandle("automation:list-macro-tiles", async () => {
  ...
  return { ok: true, root: result.root, tiles: result.macros, warnings: result.warnings };
});
...
const automationStartup = app.whenReady().then(async () => {
  ...
  if (launchRole === "automation") {
    ...
    if (startupAfterInstall) {
      if (splash && !splash.isDestroyed()) splash.close();
      await finishInstallerMacroUpdate().catch((error) => {
        bundledMacroDeploymentWarnings.push(`Could not display the setup macro warning: ${error.message}. Close SOLIDWORKS and restart Helper to retry the macro update.`);
      });
    }
...
app.on("second-instance", (_event, commandLine) => {
  if (commandLine?.includes("--after-install")) {
    automationStartup.then(() => finishInstallerMacroUpdate(true)).catch((error) => {
      logActivity("installer-macro-update-failed", { error: error.message });
    });
  }
  const win = createAutomationWindow();
  if (win && !win.isDestroyed()) {
