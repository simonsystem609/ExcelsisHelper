...
assert.match(main, /scheduleSingleDocThumbnailRetry[\s\S]{0,500}renderOnly: true/);
assert.match(main, /const skipSolidWorksTier = renderOnly \? false : await shouldSkipSolidWorksThumbnailTier\(\)/);
...
for (const source of [laserDxf, laserDxfReadOnly]) {
  ...
  assert.match(source, /ExcelsisTaskDialog Lib "comctl32\.dll" Alias "TaskDialog"/);
  assert.match(source, /RETRY = Regular DXF \+ general imported filter/);
  assert.match(source, /Function AskFullAssemblyImportedFilter\(\) As Boolean/);
  ...
  assert.match(source, /If Not IsImportedGeometryPart\(swPart\) Then Exit Function/);
  assert.match(source, /imported-filter reject scope=/);
  ...
  assert.match(source, /If g_generalImportedFilter Then[\s\S]{0,150}If IsImportedAssemblyComponent\(swComp\) Then/);
  ...
  assert.match(source, /selected reject=open-part-failed component=/);
  ...
  assert.match(source, /imported-filter hit=feature-type/);
...
const savedPreviewBlock = thumbnails.match(/# 2\) Fallback:[\s\S]*?# 3\) Optional slower fallback:/)[0];
...
const listedRuntimeScripts = [...scriptsResourceBlock.matchAll(/^\s{6}- ([^\r\n]+)$/gm)].map((match) => match[1]);
assert.deepEqual(listedRuntimeScripts, expectedRuntimeScripts, "Only the reviewed runtime scripts may be packaged.");
...
assert.match(pkg.version, /^1\.4\.12$/);
assert.equal(pkg.name, "excelsis-helper");
assert.equal(pkg.license, "GPL-3.0-only");
assert.equal(pkg.devDependencies.electron, "42.11.1");
...
// Binary macro containers can retain saved-machine paths outside the VBA text.
// Check both byte encodings; readable-source-only scans cannot detect these.
for (const name of fs.readdirSync(path.join(root, "macros")).filter((name) => name.endsWith(".swp"))) {
  const data = fs.readFileSync(path.join(root, "macros", name));
  for (const encoding of ["latin1", "utf16le"]) {
    const text = data.toString(encoding);
    assert.doesNotMatch(text, /[A-Z]:[\\/]Users[\\/]/i, `${name}: saved-user path in ${encoding}`);
    assert.doesNotMatch(text, /[A-Z]:[\\/]E_(?:HELPER|VIEWER|CAD)[\\/]/i, `${name}: saved-workspace path in ${encoding}`);
  }
}
