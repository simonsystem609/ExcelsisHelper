...
async function runThumbnailExtractionBatch(entries, options = {}) {
  ...
  // An explicit manual rebuild skips cached/embedded/shell previews and goes
  // straight to the live SOLIDWORKS render.
  ...
  try {
    ...
    // explicit manual rebuild, which must force the live SW render.
    let remaining = pairs;
    if (!renderOnly) {
    ...
    // The explicit manual rebuild may render while SOLIDWORKS is foreground;
    // that foreground guard exists only for unattended work. The PowerShell
    // tier independently honors the shared macro-run marker in every mode.
    const skipSolidWorksTier = renderOnly ? false : await shouldSkipSolidWorksThumbnailTier();
...
// The explicit per-document retry is for a visibly wrong saved preview. Force
// a fresh live SOLIDWORKS render; using the normal embedded/shell tiers here
// would simply recreate the same malformed thumbnail the user just removed.
async function scheduleSingleDocThumbnailRetry(docPath) {
  const target = String(docPath || "").trim();
  if (!target || !(await pathExists(target))) return { queued: false, regenerated: false };
  const entry = { path: target };
  const key = target.toLowerCase();
  thumbRetryState.delete(key);
  clearThumbnailRetryTimer(key);
  const jobs = await queueThumbnailExtraction([entry], {
    force: true,
    renderOnly: true,
    allowSolidWorksRender: true,
    priority: THUMB_PRIORITY_MANUAL,
  });
  const state = await readThumbnailState(thumbPathForDoc(target));
  return {
    queued: Array.isArray(jobs) && jobs.some((job) => job?.processed),
    regenerated: !state.missing && !state.blank,
  };
}
...
trustedIpcHandle("automation:retry-recent-doc-thumbnail", async (_event, docPath) => {
  ...
  thumbRetryState.delete(key);
  clearThumbnailRetryTimer(key);
  const retry = await scheduleSingleDocThumbnailRetry(target);
  return {
    ok: true,
    path: target,
    deletedThumbnail: deleted,
    queued: retry.queued,
    regenerated: retry.regenerated,
    renderOnly: true,
  };
});
