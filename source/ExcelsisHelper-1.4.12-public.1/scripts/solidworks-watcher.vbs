...
'
' Args: 0 = output file path, 1 = interval ms (optional, default 2000),
'       2 = main-process macro gate path (optional)
Option Explicit
On Error Resume Next

Dim outPath, intervalMs, macroGatePath
...
macroGatePath = ""
If WScript.Arguments.Count > 2 Then macroGatePath = CStr(WScript.Arguments(2))

Dim watcherInstanceId, connectionGeneration, watcherSessionId, nextDocumentToken
Dim connectionErrorNumber, connectionErrorDescription
...
Function IsMacroRunGateActive(gatePath)
  Dim fso, gateFile, gateText, compactText, ageSeconds
  IsMacroRunGateActive = False
  If Len(Trim(CStr(gatePath))) = 0 Then Exit Function

  Err.Clear
  Set fso = CreateObject("Scripting.FileSystemObject")
  If Err.Number <> 0 Or fso Is Nothing Then Err.Clear : Exit Function
  If Not fso.FileExists(gatePath) Then Exit Function
  Set gateFile = fso.GetFile(gatePath)
  If Err.Number <> 0 Or gateFile Is Nothing Then Err.Clear : Exit Function
  ageSeconds = DateDiff("s", gateFile.DateLastModified, Now)
  If ageSeconds < -30 Or ageSeconds > 15 Then Exit Function

  Set gateFile = fso.OpenTextFile(gatePath, 1, False)
  If Err.Number <> 0 Or gateFile Is Nothing Then Err.Clear : Exit Function
  gateText = gateFile.ReadAll
  gateFile.Close

  compactText = Replace(gateText, " ", "")
  compactText = Replace(compactText, vbTab, "")
  compactText = Replace(compactText, vbCr, "")
  compactText = Replace(compactText, vbLf, "")
  If InStr(1, compactText, """schema"":""excelsis-helper-macro-gate-v1""", vbTextCompare) = 0 Then Exit Function
  IsMacroRunGateActive = (InStr(1, compactText, """active"":true", vbTextCompare) > 0)
End Function
...
Dim sw, probe, payload, adoc, odocs, activeDoc, previousActiveDoc, activeToken, macroRunning, lastConnectedPayload
Set sw = Nothing
Set activeDoc = Nothing
Set previousActiveDoc = Nothing
lastConnectedPayload = ""

Do
  macroRunning = IsMacroRunGateActive(macroGatePath)
  If macroRunning Then
    ' A macro owns SOLIDWORKS' single-threaded automation apartment. Do not
    ' issue even a liveness property call until it releases the shared marker.
    If Len(lastConnectedPayload) > 1 Then
      payload = Replace(lastConnectedPayload, """macroRunning"":false", """macroRunning"":true")
    Else
      payload = "{""ok"":true,""connected"":false,""macroRunning"":true,""watcherSessionId"":""""," & _
        """activeDocument"":{""hasActiveDocument"":false,""title"":"""",""path"":"""",""source"":""none"",""documentToken"":"""",""identityTrusted"":false},""openDocuments"":[]}"
    End If
  Else
    ...
    If Not sw Is Nothing Then
      ...
      probe = sw.Visible
      If Err.Number <> 0 Then
        connectionErrorNumber = Err.Number
        connectionErrorDescription = Err.Description
        Err.Clear
        Set sw = Nothing
        watcherSessionId = ""
    ...
    If sw Is Nothing Then
      ...
      connectionErrorNumber = Err.Number
      connectionErrorDescription = Err.Description
      If connectionErrorNumber <> 0 Then Err.Clear : Set sw = Nothing
      If Not sw Is Nothing Then
        connectionErrorNumber = 0
        connectionErrorDescription = ""
    ...
    If sw Is Nothing Then
      payload = "{""ok"":true,""connected"":false,""watcherSessionId"":"""",""connectionErrorNumber"":" & CStr(connectionErrorNumber) & _
        ",""connectionError"":" & JsonString(connectionErrorDescription) & _
        ",""activeDocument"":{""hasActiveDocument"":false,""title"":"""",""path"":"""",""source"":""none"",""documentToken"":"""",""identityTrusted"":false},""openDocuments"":[]}"
    Else
      If Not scanInProgress Then BeginOpenDocumentsSample sw
      ' Keep a changed previous active identity for Save As relinking, but avoid
      ' rereading the same active document twice on every two-second heartbeat.
      Set previousActiveDoc = activeDoc
      adoc = GetActiveDocumentJson(sw, activeDoc, activeToken)
      If Not previousActiveDoc Is Nothing Then
        If activeDoc Is Nothing Then
          RefreshRecentActiveDocument previousActiveDoc
        ElseIf Not (previousActiveDoc Is activeDoc) Then
          RefreshRecentActiveDocument previousActiveDoc
        End If
      End If
      ContinueOpenDocumentsSample
      odocs = GetCachedOpenDocumentsJson()
      payload = "{""ok"":true,""connected"":true,""macroRunning"":false,""watcherSessionId"":" & JsonString(watcherSessionId) & _
