param(
  ...
  # Set by the manual Delete + retry action: skip shell + sw-api and go
  ...
  [switch]$RenderOnly,
  # Background batches set this while the user or a macro owns SOLIDWORKS.
  # Shell/embedded/DXF/DWG tiers remain available; live SW COM is skipped.
  [switch]$SkipSolidWorksTier,
  [string]$MacroRunMarkerPath = "",
  [ValidateRange(0, 4)][int]$MaxSolidWorksApiCalls = 1
)
...
#      manual Delete + retry action (-RenderOnly, which skips tiers 1-2).
...
function Test-SharedMacroRunMarkerActive {
  if ([string]::IsNullOrWhiteSpace($MacroRunMarkerPath)) { return $false }
  try {
    $item = Get-Item -LiteralPath $MacroRunMarkerPath -ErrorAction Stop
    if (((Get-Date) - $item.LastWriteTime).TotalHours -gt 12) { return $false }
    $marker = Get-Content -LiteralPath $MacroRunMarkerPath -Raw -Encoding UTF8 | ConvertFrom-Json
    if ([string]$marker.schema -ne "excelsis-helper-macro-run-v1") { return $false }
    $pidValue = [int]$marker.pid
...
function Test-SolidWorksNegativeDrawingPreview {
  param([string]$PngPath)
  ...
  try {
    ...
    if ($total -le 0) { return $false }
    # A drawing page should be predominantly light with sparse dark geometry.
    # This catches the known shell-preview failure where the whole sheet is
    # rendered black and its lines white, while leaving dark model previews
    # alone because this check is only called for .SLDDRW files.
    return (($dark / [double]$total) -gt 0.20 -and ($light / [double]$total) -gt 0.20)
  } catch {
    return $false
  } finally {
    if ($bmp -ne $null) { $bmp.Dispose() }
...
function Get-ThumbnailValidationError {
  ...
  if ($ext.Equals(".slddrw", [System.StringComparison]::OrdinalIgnoreCase) -and (Test-SolidWorksNegativeDrawingPreview -PngPath $PngPath)) {
    return "negative-drawing-preview"
  }
  return ""
}
...
foreach ($pair in $pairs) {
  ...
  # Manual Delete + retry (-RenderOnly) forces the reorienting render and skips the
  # cheaper tiers entirely - see the tier guards below.
  if ($RenderOnly) { $allowSolidWorksRender = $true }
  $ok = $false
  $method = ""
  $err = ""
  $solidWorksTierBlocked = [bool]$SkipSolidWorksTier -or (Test-SharedMacroRunMarkerActive)
  ...
  if (
    (-not $ok) -and
    (-not $RenderOnly) -and
    (-not $solidWorksTierBlocked) -and
    $script:SolidWorksApiCalls -lt $MaxSolidWorksApiCalls
  ) {
    try {
  ...
  if ((-not $ok) -and $allowSolidWorksRender -and (-not $solidWorksTierBlocked)) {
    try {
      if (Get-ThumbnailViaSolidWorksRender -SwPath $itemPath -OutPng $itemOut -Size $ThumbSize) {
        ...
        if (
          $renderExt.Equals(".slddrw", [System.StringComparison]::OrdinalIgnoreCase) -and
          ((Test-SolidWorksBadColorPreview -PngPath $itemOut) -or (Test-SolidWorksNegativeDrawingPreview -PngPath $itemOut))
        ) {
          Remove-Item -LiteralPath $itemOut -Force -ErrorAction SilentlyContinue
          if ($err) { $err += " | " }
