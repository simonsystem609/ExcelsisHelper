...
node --check recent-doc-maintenance.cjs
...
upstream checksums verify those inputs. Electron 42.11.1 has no npm lifecycle
...
The transfer source intentionally contains no `.npmrc`. Pass
`--legacy-peer-deps` explicitly to `npm.cmd ci`; it omits electron-builder's
optional Squirrel.Windows target plugin from the resolved dependency tree.
This project configures only NSIS, and the omitted plugin is not needed to
install dependencies, test, package, or inspect this release.
...
The locked `@xmldom/xmldom` 0.8.15 and `fast-uri` 3.1.7 updates clear current
build-dependency advisories. Electron 42.11.1 includes the upstream major-42
security backports. No new runtime npm modules are introduced.

Do not launch the application merely to inspect a package. The build creates
one universal, preset-free installer:

- dist\Excelsis Helper-Setup-1.4.12.exe
- dist\Excelsis Helper-Setup-1.4.12.exe.blockmap
- dist\win-unpacked\
...
~~~powershell
Get-FileHash -Algorithm SHA256 '.\dist\Excelsis Helper-Setup-1.4.12.exe'
Get-AuthenticodeSignature '.\dist\Excelsis Helper-Setup-1.4.12.exe'
