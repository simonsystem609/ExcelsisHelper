...
This inventory covers Excelsis Helper 1.4.12.
...
`thumbnail-scheduler.cjs`, `recent-doc-maintenance.cjs`, and the bounded
SOLIDWORKS watcher changes are project-authored code and do not add third-party
runtime material.
...
The installer contains compiled `.swp` forms of these eight project-owned
macros. They are generated locally from the corresponding reviewed `.swb`
source. Reviewed SWB source and build-ready SWP artifacts are both included;
the application build packages the SWPs directly. Macro changes can be saved
as matching SWPs in a compatible SOLIDWORKS macro editor. No vendor SDK or
development program is distributed in source or application resources.

The public release neutralizes saved-machine path fields in the eight macro
containers. This is a metadata-only change: field lengths, every VBA source
stream, performance caches, references, and module identifiers are preserved.
