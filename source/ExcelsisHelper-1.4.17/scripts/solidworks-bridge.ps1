...
$macroRunMarkerRoot = Join-Path ([Environment]::GetFolderPath([Environment+SpecialFolder]::MyDocuments)) "Excelsis Helper"
$macroRunMarkerPath = Join-Path $macroRunMarkerRoot "macro-running.json"
$macroRunMarkerOwned = $false

function Start-MacroRunMarker {
  if ($Action -ne "run") { return }
  try {
    New-Item -ItemType Directory -Path $macroRunMarkerRoot -Force | Out-Null
    $payload = [ordered]@{
      schema = "excelsis-helper-macro-run-v1"
      pid = $PID
      startedAt = (Get-Date).ToUniversalTime().ToString("o")
      macroName = [System.IO.Path]::GetFileName($MacroPath)
    } | ConvertTo-Json -Compress
    $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    [System.IO.File]::WriteAllText($macroRunMarkerPath, $payload, $utf8NoBom)
    $script:macroRunMarkerOwned = $true
  } catch {
    $script:macroRunMarkerOwned = $false
  }
}

function Stop-MacroRunMarker {
  if (-not $script:macroRunMarkerOwned) { return }
  try {
    if (-not (Test-Path -LiteralPath $macroRunMarkerPath -PathType Leaf)) { return }
    $marker = Get-Content -LiteralPath $macroRunMarkerPath -Raw -Encoding UTF8 | ConvertFrom-Json
    if ([int]$marker.pid -eq $PID) {
      Remove-Item -LiteralPath $macroRunMarkerPath -Force
    }
  } catch {
    # A newer bridge may own the marker; leave it untouched.
  } finally {
    $script:macroRunMarkerOwned = $false
  }
}
...
Function DxfResultValue(resultText, key)
  Dim lines, line, prefix, lineIndex
  DxfResultValue = ""
  prefix = LCase(CStr(key) & "=")
  lines = Split(CStr(resultText), vbLf)
  For lineIndex = 0 To UBound(lines)
    line = Replace(CStr(lines(lineIndex)), vbCr, "")
    If LCase(Left(line, Len(prefix))) = prefix Then
      DxfResultValue = Mid(line, Len(prefix) + 1)
      Exit Function
    End If
  Next
End Function

Function ReadExcelsisDxfResult(resultPath, beforeModified, ByRef resultStatus, ByRef resultError, ByRef resultTrace)
  Dim fso, resultFile, changedAt, resultText, errNumber, errDescription
  ReadExcelsisDxfResult = False
  resultStatus = ""
  resultError = ""
  resultTrace = ""
  ...
  Err.Clear
  Set fso = CreateObject("Scripting.FileSystemObject")
  If Err.Number <> 0 Or Not fso.FileExists(resultPath) Then
    Err.Clear
    Exit Function
  End If
  Set resultFile = fso.GetFile(resultPath)
  changedAt = CDbl(resultFile.DateLastModified)
  If Err.Number <> 0 Or changedAt <= CDbl(beforeModified) Then
    Err.Clear
    Exit Function
  End If

  Set resultFile = fso.OpenTextFile(resultPath, 1, False)
  ...
  resultStatus = UCase(Trim(DxfResultValue(resultText, "status")))
  If resultStatus <> "OK" And resultStatus <> "ERROR" Then Exit Function

  errNumber = Trim(DxfResultValue(resultText, "errorNumber"))
  errDescription = Trim(DxfResultValue(resultText, "errorDescription"))
  resultTrace = Trim(DxfResultValue(resultText, "tracePath"))
  If resultStatus = "ERROR" Then
    resultError = "DXF export failed"
    If Len(errNumber) > 0 Then resultError = resultError & " (VBA " & errNumber & ")"
    If Len(errDescription) > 0 Then resultError = resultError & ": " & errDescription
    If Len(resultTrace) > 0 Then resultError = resultError & " Trace: " & resultTrace
  End If
  ReadExcelsisDxfResult = True
End Function
...
Dim sw, startedSolidWorks, connectedError, connectionFailed
...
ElseIf sw Is Nothing Then
  connectedError = "SOLIDWORKS returned no application object."
  connectionFailed = True
End If
...
Dim dxfResultPath, dxfResultBeforeModified, dxfResultMonitored
dxfResultPath = ""
dxfResultBeforeModified = -1
dxfResultMonitored = False
If extensionName = "swp" And (LCase(fileBase) = "dxf_v16" Or LCase(fileBase) = "dxf_v16_rofriendly") Then
  Dim dxfShell, dxfResultFile
  Set dxfShell = CreateObject("WScript.Shell")
  dxfResultPath = dxfShell.ExpandEnvironmentStrings("%TEMP%") & "\Excelsis_DXF_last_result.txt"
  If fso.FileExists(dxfResultPath) Then
    Err.Clear
    Set dxfResultFile = fso.GetFile(dxfResultPath)
    dxfResultBeforeModified = CDbl(dxfResultFile.DateLastModified)
    If Err.Number <> 0 Then dxfResultBeforeModified = -1 : Err.Clear
  End If
  dxfResultMonitored = True
End If
...
Else
  optionsValue = 1
  AddUnique moduleCandidates, moduleName
  ' Known compiled templates also work when GetMacroMethods returns nothing.
  If LCase(fileBase) = "backupassembly_v1" Or LCase(fileBase) = "radius_v9" Then AddUnique moduleCandidates, "Test11"
  If LCase(fileBase) = "pdf_v1" Then AddUnique moduleCandidates, "DXF_v161"
...
Dim dxfResultFresh, dxfResultStatus, dxfResultError, dxfResultTrace
dxfResultFresh = False
dxfResultStatus = ""
dxfResultError = ""
dxfResultTrace = ""
If dxfResultMonitored Then
  dxfResultFresh = ReadExcelsisDxfResult(dxfResultPath, dxfResultBeforeModified, dxfResultStatus, dxfResultError, dxfResultTrace)
  If dxfResultFresh And dxfResultStatus = "ERROR" Then selectedOk = False
End If

Dim errorText
If dxfResultFresh And dxfResultStatus = "ERROR" Then
  errorText = dxfResultError
ElseIf selectedOk Then
  errorText = ""
Else
...
WriteUtf8 outputPath, "{""ok"":" & JsonBool(selectedOk) & _
  ...
  ",""macroResultStatus"":" & JsonString(dxfResultStatus) & _
  ",""macroResultPath"":" & JsonString(dxfResultPath) & _
...
function Invoke-KillSolidWorksInstances {
  $initialProcs = @(Get-Process -Name "SLDWORKS" -ErrorAction SilentlyContinue)
  $seenHints = @{}
  $attemptErrors = @{}
  foreach ($proc in $initialProcs) {
    $hint = Get-SolidWorksProcessHint $proc -IncludeDiagnostics
    $seenHints[[string]$hint.id] = $hint
  }

  # Stop only exact SLDWORKS.exe PIDs. The later taskkill passes catch child
  # processes and a process that survives the normal PowerShell force-stop.
  for ($pass = 1; $pass -le 3; $pass++) {
    $current = @(Get-Process -Name "SLDWORKS" -ErrorAction SilentlyContinue)
    if ($current.Count -eq 0) { break }
    foreach ($proc in $current) {
      $hint = Get-SolidWorksProcessHint $proc -IncludeDiagnostics
      $pidKey = [string]$hint.id
      $seenHints[$pidKey] = $hint
      try {
        if ($pass -eq 1) {
          Stop-Process -Id $hint.id -Force -ErrorAction Stop
        } else {
          $taskkill = Join-Path $env:SystemRoot "System32\taskkill.exe"
          & $taskkill "/PID" $pidKey "/T" "/F" 2>$null | Out-Null
          if ($LASTEXITCODE -ne 0 -and $null -ne (Get-Process -Id $hint.id -ErrorAction SilentlyContinue)) {
            throw "taskkill exited with code $LASTEXITCODE"
          }
        }
      } catch {
        $attemptErrors[$pidKey] = $_.Exception.Message
      }
    }
  ...
  $survivorIds = @{}
  $survivors = @($survivorProcs | ForEach-Object {
    $hint = Get-SolidWorksProcessHint $_ -IncludeDiagnostics
    $survivorIds[[string]$hint.id] = $true
    $seenHints[[string]$hint.id] = $hint
    $hint
  })
  $killed = New-Object System.Collections.Generic.List[object]
  $failed = New-Object System.Collections.Generic.List[object]
  foreach ($entry in $seenHints.GetEnumerator()) {
    if ($survivorIds.ContainsKey([string]$entry.Key)) {
      $errorText = if ($attemptErrors.ContainsKey([string]$entry.Key)) { $attemptErrors[[string]$entry.Key] } else { "Process survived all force-stop attempts." }
      $failed.Add(([ordered]@{ id = $entry.Value.id; error = $errorText; hint = $entry.Value }))
    } else {
      $killed.Add($entry.Value)
    }
  }
  return [ordered]@{
    schema = "excelsis-kill-sw-v1"
    ok = ($survivors.Count -eq 0)
    totalInstances = $seenHints.Count
...
try {
  # The main process requires an explicit user confirmation, then this action
  # force-stops every exact SLDWORKS.exe instance and reports any survivors.
