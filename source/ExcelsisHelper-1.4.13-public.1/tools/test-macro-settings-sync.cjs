...
async function test() {
  ...
  vm.runInContext([
    ...
    "this.api = { applyMacroSettings, ensureBundledMacros, macroSettingsSyncStatus };",
  ].join("\n"), context);
  ...
  console.log("Macro settings fresh-install, atomic update, lock reporting, repair, and concurrent-save tests passed.");
}
