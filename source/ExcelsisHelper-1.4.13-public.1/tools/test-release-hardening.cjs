...
assert.match(radius, /CollectSelectedFaceTokens\(swModel\)/);
assert.match(radius, /selectionType = selMgr\.GetSelectedObjectType3\(i, -1\)/);
assert.match(radius, /If selectionType = swSelFACES Then Set selectedFace = selMgr\.GetSelectedObject6\(i, -1\)/);
assert.match(radius, /ReferencedConfiguration/);
assert.match(radius, /GetCorrespondingEntity2\(assemblyFace\)/);
assert.match(radius, /area = CDbl\(face\.GetArea\)/);
assert.match(radius, /GetLargestFace\(swPart\)/);
assert.match(radius, /vEdges = swFace\.GetEdges/);
assert.doesNotMatch(radius, /vEdges = swBody\.GetEdges/);
assert.match(radius, /chunkSize = 8\s+madeGroup = False/);
assert.match(radius, /groupCount = 0\s+Erase groupEdges\s+Erase groupKeys\s+For i = 1 To edgeCount/);
...
const expectedRuntimeScripts = [
  ...
  "capture-sw-viewport.ps1",
  "capture-sw-viewport.vbs",
...
assert.match(pkg.version, /^1\.4\.13$/);
