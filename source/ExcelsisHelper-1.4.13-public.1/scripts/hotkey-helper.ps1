...
public static class ExcelsisHotkeyHelper {
  ...
  private static readonly WinEventDelegate ForegroundProc = ForegroundChanged;
  ...
  private static extern IntPtr SetWinEventHook(uint eventMin, uint eventMax, IntPtr eventHook,
    WinEventDelegate eventProc, uint processId, uint threadId, uint flags);
  ...
  private static bool ForegroundProcessIsSolidWorks() {
    try {
      uint pid;
      IntPtr hwnd = GetForegroundWindow();
      if (hwnd == IntPtr.Zero) return false;
      ...
      if (pid == 0) return false;
      using (Process proc = Process.GetProcessById((int)pid)) {
        return String.Equals(proc.ProcessName, "SLDWORKS", StringComparison.OrdinalIgnoreCase);
      }
    } catch {
      return false;
    }
