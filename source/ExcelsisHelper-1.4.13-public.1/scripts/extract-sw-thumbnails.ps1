param(
  ...
  # breaker (see ShellTierDisabled below) - skips straight to saved sw-api preview
...
# Manual current-viewport capture is separate: capture-sw-viewport.ps1.
# Background extraction never opens or changes a document's view.
#
...
# reject a blank sw-api result so it is never saved (the caller then
...
# REST of this batch (falling through to sw-api/no-thumbnail
...
foreach ($pair in $pairs) {
  ...
  if ((-not $ok) -and (-not $script:ShellTierDisabled)) {
