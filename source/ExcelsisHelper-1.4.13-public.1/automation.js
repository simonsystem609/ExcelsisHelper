...
const ui = {
  ...
  settingsWorkLoggingEnabled: document.getElementById("settingsWorkLoggingEnabled"),
...
const SOLIDWORKS_STATUS_INTERVAL_MS = 2000;
...
function updateSolidWorksKillButton(health) {
  ...
  ui.killSolidWorksBtn.title = ta(canKill
    ? "Force-stop all SLDWORKS.exe processes"
    : "No SLDWORKS.exe process is available");
}

function renderSolidWorksHealth(result) {
  const health = result?.solidWorksConnection || result?.solidWorksHealth || null;
  state.lastSolidWorksHealth = health;
  updateSolidWorksKillButton(health);
  if (!ui.swHealthStatus) return;

  const stateKey = ["connected", "com-unresponsive", "disconnected", "checking"].includes(health?.state)
    ? health.state
    : "checking";
  ui.swHealthStatus.classList.remove("connected", "com-unresponsive", "disconnected", "checking");
  ui.swHealthStatus.classList.add(stateKey);
  ui.swHealthStatus.textContent = t(health?.label || "SW COM: checking");
  ui.swHealthStatus.title = health?.message || ui.swHealthStatus.textContent;
}
...
function setStatus(result) {
  ...
  const connectionState = result.connectionState
    || result.solidWorksConnection?.state
  ...
  } else if (connectionState === "com-unresponsive" || connectionState === "checking") {
    ui.swStatus.textContent = t(connectionState === "com-unresponsive" ? "COM unresponsive" : "Not checked");
    ui.swStatus.classList.add("idle");
  } else {
  ...
  if (connectionState === "com-unresponsive") {
    const last = result.lastKnownActiveDocument?.hasActiveDocument
      ? result.lastKnownActiveDocument
      : state.lastSolidWorksDocument;
    if (last?.hasActiveDocument) {
      const full = displayPathOf(last.path);
      setActiveDocText(`${t("COM unresponsive. Last seen:")} ${last.title}${full ? `  ${full}` : ""}`);
    } else {
      setActiveDocText(t("COM unresponsive. No last-known document."));
    }
    return;
  }
...
function renderMacroTiles() {
  ...
  ui.macroFolderState.textContent = state.macroRoot
    ...
    : "Macro folder: Documents\\Excelsis Helper\\Macros";
  if (state.macroWarnings.length) ui.macroFolderState.textContent += `\n${state.macroWarnings.join("\n")}`;
  ui.macroFolderState.classList.toggle("muted", !state.macroWarnings.length);
...
async function refreshRecentDocsList() {
  ...
  try {
    ...
    const canToggleAll = Boolean(result.canToggleAll || result.limited || recentDocsState.showAll);
    if (ui.recentDocsMore) ui.recentDocsMore.classList.toggle("hidden", !canToggleAll);
    if (ui.showAllRecentDocsBtn) {
      ui.showAllRecentDocsBtn.textContent = recentDocsState.showAll ? t("Show first 50") : t("Show all");
      ui.showAllRecentDocsBtn.setAttribute("aria-expanded", String(recentDocsState.showAll));
    }
    if (ui.recentDocsMoreState) {
      const shown = (result.entries || []).length;
      ui.recentDocsMoreState.textContent = canToggleAll
...
async function retryRecentDocThumbnail(entry) {
  if (!entry?.path) return;
  hideRecentDocContextMenu();
  try {
    let result;
    if (typeof api.retryRecentDocThumbnail === "function") {
      result = await api.retryRecentDocThumbnail(entry.path);
    } else {
    ...
    if (!result?.ok) {
      ...
      window.alert(result?.error || "Could not capture the SOLIDWORKS viewport. Existing thumbnail kept.");
      return;
    }
    recentDocsState.lastSignature = "";
    log("Thumbnail replaced with the current SOLIDWORKS viewport.", {
      path: entry.path,
      regenerated: result.regenerated,
      viewportOnly: result.viewportOnly,
    });
    await refreshRecentDocsList();
    if (document.getElementById("docSearchView")?.classList.contains("active")) {
      docSearchState.lastSignature = "";
      await refreshDocSearch();
    }
  } catch (error) {
...
async function copyPathForEntry(entry, includeDocumentName) {
  if (!entry?.path) return;
  hideRecentDocContextMenu();
  try {
    const result = await api.copyDocumentPath(entry.path, { includeDocumentName });
    if (!result?.ok) {
      log("Could not copy document path.", result);
      return;
    }
    log(includeDocumentName ? "Document path copied." : "Containing folder path copied.", {
      path: result.clipboardText || entry.path,
    });
  } catch (error) {
...
function copyPathSubmenuMarkup() {
  return `
    <div class="recent-docs-context-menu-item-with-submenu">
      <button type="button" class="context-menu-submenu-trigger" data-submenu-trigger="copy-path" aria-haspopup="menu" aria-expanded="false">
        <span>${escapeHtml(t("Copy path"))}</span>
        <span class="context-menu-submenu-arrow" aria-hidden="true">&rsaquo;</span>
      </button>
      <div class="recent-docs-context-submenu" role="menu" aria-label="${escapeAttr(t("Copy path"))}">
        <button type="button" role="menuitem" data-action="copy-path-folder">${escapeHtml(t("No document name"))}</button>
        <button type="button" role="menuitem" data-action="copy-path-document">${escapeHtml(t("Include document name"))}</button>
      </div>
    </div>
  `;
}

function positionRecentDocContextMenu(menu, event) {
  ...
  const submenuItem = menu.querySelector(".recent-docs-context-menu-item-with-submenu");
  const submenu = submenuItem?.querySelector(".recent-docs-context-submenu");
  if (submenuItem && submenu) {
    const submenuWidth = Math.max(184, submenu.getBoundingClientRect().width || 0);
    const placedRect = menu.getBoundingClientRect();
    submenuItem.classList.toggle("submenu-left", placedRect.right + submenuWidth > window.innerWidth - pad);
  }
}

function contextMenuAction(clickEvent) {
  clickEvent.stopPropagation();
  const button = clickEvent.target?.closest?.("button");
  if (!button) return "";
  if (button.dataset.submenuTrigger) {
    const wrapper = button.closest(".recent-docs-context-menu-item-with-submenu");
    const isOpen = wrapper?.classList.toggle("submenu-open") || false;
...
function showRecentDocContextMenu(event, entry) {
  ...
  menu.addEventListener("click", (clickEvent) => {
    const action = contextMenuAction(clickEvent);
    if (action === "open-folder") openContainingFolderForEntry(entry);
    if (action === "copy-path-folder") copyPathForEntry(entry, false);
    if (action === "copy-path-document") copyPathForEntry(entry, true);
  ...
  positionRecentDocContextMenu(menu, event);
...
function updateWorklogExportPreview() {
  ...
  if (ui.confirmWorklogExportBtn) ui.confirmWorklogExportBtn.disabled = workLoggerState.enabled === false || preview.exported <= 0;
}
...
async function openWorklogExportDialog(options = {}) {
  if (workLoggerState.enabled === false) return;
...
function renderWorkLoggerCountingStatus(counter = null) {
  ...
  } else if (status.code === "disabled") {
    headline = t("Work logging is disabled");
    message = t("Work logging and export are disabled in Settings.");
  }
  const meta = [];
...
function renderWorkLoggingControls(enabled) {
  workLoggerState.enabled = enabled;
  for (const button of ui.workLoggerList?.querySelectorAll(".worklog-adjust-btn") || []) {
    button.disabled = !enabled;
  }
  if (ui.exportWorkLoggerBtn) ui.exportWorkLoggerBtn.disabled = !enabled;
  if (ui.exportLastDayWorkLoggerBtn) {
    ui.exportLastDayWorkLoggerBtn.disabled = !enabled || !workLoggerState.lastDayBackup;
  }
  if (ui.worklogAutoExportSkip) ui.worklogAutoExportSkip.disabled = !enabled;
  if (!enabled) setWorklogExportDialogOpen(false);
}

function renderAutoExportStatus(ax) {
  if (ax) renderWorkLoggingControls(ax.enabled !== false);
...
async function refreshLastDayAvailability() {
  ...
  try {
    ...
    ui.exportLastDayWorkLoggerBtn.disabled = workLoggerState.enabled === false || !available;
...
async function killSolidWorks() {
  ...
  if (!state.lastSolidWorksHealth?.canKill) {
    log("No SLDWORKS.exe process is available to stop.", state.lastSolidWorksHealth || {});
    refreshSolidWorksStatus().catch(() => {});
    return;
  }
  const confirmed = window.confirm("Force-stop all SLDWORKS.exe processes? Unsaved SOLIDWORKS work may be lost.");
  if (!confirmed) return;
...
const I18N_HU = {
  ...
  "No document name": "Dokumentumnév nélkül",
  "Include document name": "Dokumentumnévvel",
  ...
  "COM unresponsive": "A COM nem v\u00e1laszol",
  ...
  "COM unresponsive. Last seen:": "A COM nem v\u00e1laszol. Utolj\u00e1ra l\u00e1tva:",
  "COM unresponsive. No last-known document.": "A COM nem v\u00e1laszol. Nincs utolj\u00e1ra ismert dokumentum.",
  "SW COM: connected": "SW COM: kapcsol\u00f3dva",
  "SW COM: unresponsive": "SW COM: nem v\u00e1laszol",
  "SW: not running": "SW: nem fut",
  "SW COM: checking": "SW COM: ellen\u0151rz\u00e9s",
  ...
  "Show first 50": "Els\u0151 50 mutat\u00e1sa",
  ...
  "Enable work logging and export": "Munkaidő-naplózás és export engedélyezése",
  "ERP worklog user name (optional)": "ERP munkaidő-felhasználónév (opcionális)",
  "Windows user name": "Windows-felhasználónév",
  "Work logging is disabled": "A munkaidő-naplózás ki van kapcsolva",
  "Work logging and export are disabled in Settings.": "A munkaidő-naplózás és az export ki van kapcsolva a Beállításokban.",
  "Macro settings saved for the next run.": "A makróbeállítások a következő futtatásra elmentve.",
  "Macro settings could not be synchronized.": "A makróbeállítások szinkronizálása sikertelen.",
  "Macro settings file:": "Makróbeállítások fájlja:",
...
const I18N_HU_ATTR = {
  ...
  "Force-stop all SLDWORKS.exe processes": "Az \u00f6sszes SLDWORKS.exe folyamat k\u00e9nyszer\u00edtett le\u00e1ll\u00edt\u00e1sa",
  "No SLDWORKS.exe process is available": "Nem \u00e9rhet\u0151 el SLDWORKS.exe folyamat",
...
function renderSettings(settings) {
  ...
  if (ui.settingsWorkLoggingEnabled) ui.settingsWorkLoggingEnabled.checked = settings.erp?.workLoggingEnabled !== false;
  renderWorkLoggingControls(settings.erp?.workLoggingEnabled !== false);
...
function readSettingsForm() {
  return {
    ...
    erp: {
      workLoggingEnabled: ui.settingsWorkLoggingEnabled?.checked !== false,
    ...
    macros: {
      drawingTemplate: state.settings?.macros?.drawingTemplate || "",
...
function applySettingsGroupDefaults(group) {
  ...
  switch (group) {
    ...
    case "activity":
      ...
      return true;
    case "erp":
      if (ui.settingsWorkLoggingEnabled) ui.settingsWorkLoggingEnabled.checked = defaults.erp?.workLoggingEnabled !== false;
...
async function refreshSolidCamLoadStatus({ force = false } = {}) {
  if (!ui.solidCamLoadStatus) return;
  if (state.solidCamStatusInFlight) return;
  const cam = state.solidCamSettings || {};
...
function renderSettingsState(result, action = "loaded") {
  ...
  const macroLine = macro
    ? `\n${t(macro.synced === true ? "Macro settings saved for the next run." : "Macro settings could not be synchronized.")}\n${t("Macro settings file:")} ${macro.settingsPath || ""}`
    : "";
  ...
  if (warningLine && action !== "loaded") ui.settingsState.scrollIntoView({ block: "nearest" });
}
...
async function loadSettings() {
  try {
    ...
    if (ui.settingsErpWorklogUserName && result.defaultErpUserName) {
      ui.settingsErpWorklogUserName.placeholder = result.defaultErpUserName;
    }
...
async function saveSettings() {
  ...
  try {
    ...
    ui.settingsState.classList.remove("muted");
    ui.settingsState.scrollIntoView({ block: "nearest" });
...
// Work logging runs in the main-process helpers even while this tray window is
// hidden. These renderer polls only repaint cached state. The optional SolidCAM
// badge stays visibility-gated and its main-process probe is cached/single-flight.
...
if (ui.showAllRecentDocsBtn) {
  ui.showAllRecentDocsBtn.addEventListener("click", () => {
    recentDocsState.showAll = !recentDocsState.showAll;
    recentDocsState.lastSignature = "";
    recentDocsState.resetRender = true;
    refreshRecentDocsList();
  });
